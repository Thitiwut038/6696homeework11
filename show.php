...
<body>
    ...
    <h1>แสดงข้อมูลรถยนต์</h1>
    <h2>พัฒนาโดย 664485038 นายธิติวุฒิ ศิริทรัพย์ </h2>
        ...
        <tbody>
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo '<td><a type="button" href="show.php?action_even=del&employee_id=' . $row['car_id'] . '" title="ลบข้อมูล" onclick="return confirm(\'ต้องการจะลบข้อมูลรายชื่อ ' . $row['car_id'] . ' ' . $row['brand'] . ' ' . $row['model'] . '?\')" class="btn btn-danger btn-sm"> ลบข้อมูล </a>  
                    
                    <a type="button" href="edit.php?action_even=edit&car_id=' . $row['car_id'] . '" 
                title="แก้ไขข้อมูล" onclick="return confirm(\'ต้องการจะแก้ไขข้อมูลรายชื่อ ' . $row['car_id'] . ' ' . $row['brand'] . ' ' . $row['model'] . '?\')" class="btn btn-primary btn-sm"> แก้ไขข้อมูล </a> </td>';
